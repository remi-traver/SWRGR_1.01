...
bool saves_poison_death( int level, CHAR_DATA * victim )
{
   ...
   */
    ...
    /*Added from SWGD*/
    int save = number_percent();         /* random base value */

    save -= victim->saving_poison_death; /* Negative values are better */
    save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
    save -= level;                       /* Take away the level of the attack */
    save = UMAX(5, save);              /* Make sure they have at least a one in twenty shot */
    if (victim->subclass == SUBCLASS_SURVIVOR || victim->subclass == SUBCLASS_FORCE_SENSITIVE)
        save += 5;                        /* Make that 2 in twenty for the subs */

    save = URANGE(5, save, 95);        /* Cap their saves at 19/20 */
    return chance(victim, save);       /* Incorporate luck roll */
...
bool saves_wands( int level, CHAR_DATA * victim )
{
    ...
    */
    ...
    save -= victim->saving_wand;         /* Negative values are better */
    save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
    save -= level;                       /* Take away the level of the attack */
    save = UMAX(5, save);              /* Make sure they have at least a one in twenty shot */
    if (victim->subclass == SUBCLASS_SURVIVOR || victim->subclass == SUBCLASS_FORCE_SENSITIVE)
...
bool saves_para_petri( int level, CHAR_DATA * victim )
{
   ...
   */
    ...
    save -= victim->saving_para_petri;   /* Negative values are better */
    save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
    save -= level;                       /* Take away the level of the attack */
    save = UMAX(5, save);              /* Make sure they have at least a one in twenty shot */
    if (victim->subclass == SUBCLASS_SURVIVOR || victim->subclass == SUBCLASS_FORCE_SENSITIVE)
...
bool saves_breath( int level, CHAR_DATA * victim )
{
   ...
   */
   ...
   save -= victim->saving_breath;       /* Negative values are better */
   save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
   save -= level;                       /* Take away the level of the attack */
   save = UMAX(5, save);              /* Make sure they have at least a one in twenty shot */
   if (victim->subclass == SUBCLASS_SURVIVOR || victim->subclass == SUBCLASS_FORCE_SENSITIVE)
...
bool saves_spell_staff( int level, CHAR_DATA * victim )
{
   ...
   */
    ...
    save -= victim->saving_spell_staff;  /* Negative values are better */
    save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
    save -= level;                       /* Take away the level of the attack */
    save -= victim->saving_poison_death; /* Negative values are better */
    save += victim->top_level / 2;         /* Saving gear isn't common, give them a chance */
...
void do_cast( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !IS_NPC( ch ) && ch->subclass != SUBCLASS_FALSE_PROPHET && abs( ch->alignment - skill->alignment ) > 1000 )
   {
...
ch_ret spell_black_lightning( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   dam = level + 200 + number_percent() * 2;
...
ch_ret spell_darkmatter(int sn, int level, CHAR_DATA* ch, void* vo)
{
    CHAR_DATA* victim = (CHAR_DATA*)vo;
    int dam;

    dam = 6666;
    act(AT_MAGIC, "$n raises $s hands, and a stream of dark matter flies from between them!!!",
        ch, NULL, NULL, TO_ROOM);
    act(AT_MAGIC, "You raise your hands, and a generate a stream of Dark Matter!",
        ch, NULL, NULL, TO_CHAR);
    return damage(ch, victim, dam, sn);
}


void shift_alignment(CHAR_DATA* ch, int sn)
{
    short alignshift;
    ...
    {
    default:
    case SP_NONE:    alignshift = 1; break;
    case SP_MINOR:   alignshift = 2; break;
    case SP_GREATER: alignshift = 3; break;
    case SP_MAJOR:   alignshift = 4; break;
    }
    alignshift *= (skill->alignment / 200);
    if (skill->type == SKILL_SPELL && IS_VALID_SN(sn))
    {
        if (alignshift > 0)
            jedi_bonus(ch);
    ...
    ch->alignment += alignshift;
    if (!IS_NPC(ch) && ch->subclass == SUBCLASS_JEDI)
        min = 400;
    if (!IS_NPC(ch) && (ch->subclass == SUBCLASS_SITH_HUNTER || ch->subclass == SUBCLASS_FALSE_PROPHET))
        max = -400;
    ch->alignment = URANGE(min, ch->alignment, max);
}
