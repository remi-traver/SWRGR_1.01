...
bool can_dual( CHAR_DATA * ch )
{
   ...
   {
       send_to_char("Your method of fighting prevents you from wielding weapons.\n\r", ch);
       return FALSE;
   }
   return TRUE;
}
...
*/
      ...
      case ITEM_WIELD:
          ...
          {
              if (IS_OBJ_STAT(tmpobj, ITEM_TWO_HANDED))
              {
                  send_to_char("Your current weapon requires you to use both hands!\n\r", ch);
                  return;
              }
              if (IS_OBJ_STAT(obj, ITEM_TWO_HANDED))
              {
                  send_to_char("The weapon you are trying to wear is two handed, and you already have a weapon equipped\n\r", ch);
                  return;
              }
          }
          if ((tmpobj = get_eq_char(ch, WEAR_HOLD)) != NULL)
          {
              ...
              if (IS_OBJ_STAT(tmpobj, ITEM_TWO_HANDED))
              {
                  send_to_char("The item you are holding requires both hands to use.\n\r", ch);
                  return;
              }
          }
          if (ch->subclass == SUBCLASS_MARTIST)
