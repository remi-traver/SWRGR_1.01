...
int max_level( CHAR_DATA * ch, int ability )
{
   ...
   if( ability == COMBAT_ABILITY )
   {
      ...
      if( ch->main_ability == HUNTING_ABILITY )
         level = 40;
      if (ch->main_ability == MEDICAL_ABILITY)
          level += 30;
      //if (ch->subclass) == )   //Add Subclass if they get combat bump
   ...
   {
      //level = 0;
      if( IS_SET(ch->pcdata->flags, PCFLAG_BOUNTY_SPEC))
         level += 10;
      if( ch->main_ability == ability )
         level = 105;
      if( ch->race == RACE_WOOKIEE )
   ...
   {
   default: break;
   case SUBCLASS_JACKOFTRADES:
       if (ability != FORCE_ABILITY)
           level += 10;
       break;
   case SUBCLASS_SOLDIER:
       if (ability == COMBAT_ABILITY || ability == HUNTING_ABILITY)
           level += 25;
       break;
   case SUBCLASS_MECHANIC:
       if (ability == ENGINEERING_ABILITY || ability == PILOTING_ABILITY)
           level += 25;
       break;
   case SUBCLASS_SITH_HUNTER:
       if (ability == HUNTING_ABILITY)
           level += 50;
       break;
...
/*    Commented out to insert SWL code base
...
void advance_level(CHAR_DATA* ch, int ability)
{
    ...
    if (ability == SMUGGLING_ABILITY /*|| ability == ESPIONAGE_ABILITY */)
    {
        add_hp /= 3;
        add_mv /= 3;
        ch->gold += add_credits;
    ...
    if (!IS_NPC(ch))
    { /*   //Comment out the random force roll in SWGD - random 1-100 level main.
        if (ability == (get_primary_ability(ch->main_ability)) && ch->skill_level[ability] == ch->force_roll
            && !IS_SET(ch->pcdata->flags, PCFLAG_FORCEROLL))
        {
            force_roll(ch);
        } */
        do_save(ch, "");
   ...
   else
   {
      ...
      switch ( ch->position )
      {
         case POS_SLEEPING:
            ...
            break;
         case POS_RESTING:
	        gain += ( int )( get_curr_wis( ch ) * 2 );
            break;
         default: gain += get_curr_wis(ch);
            break;
      }
...
void reboot_check( time_t reset )
{
   ...
   if( new_boot_time_t <= current_time )
   {
      ...
      echo_to_all( AT_YELLOW, "Your life is burned to ashes from an overpowering darkness in the force,"
                   "yet, you are mysteriously reconstructed like a phoenix from the ashes.\n\r", ECHOTAR_ALL );
