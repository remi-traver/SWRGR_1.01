...

{
    ...
    if (value < -32700 || value > 100)
    {
        send_to_char("&CAward range is -32700 to 100\n\r", ch);
        return;
    }
    ...
    if (!str_cmp(arg1, "all"))
    {
        ...
        sprintf(buf, "&wYou have awarded everyone &Y%ld&w Quest Points!\n\r", value);
        send_to_char(buf, ch);
        ...
        for (vch = first_char; vch; vch = vch_next)
        {
            ...
            if (!IS_NPC(vch) && !IS_IMMORTAL(vch) && vch->forsaken == 0)
            {

                if (value > 0)
                {
                    vch->pcdata->quest_curr += value;
                    vch->pcdata->quest_accum += value;
                    sprintf(buf, "&wYou have been awarded &Y%ld&w Quest Points by  %s!\n\r", value, ch->name);
                ...
                {
                    vch->pcdata->quest_curr += value;
                    sprintf(buf, "&wYou have been penalized &Y%ld&w Quest Points by %s!\n\r", value, ch->name);
                    send_to_char(buf, vch);
                    do_save(vch, "");
                }
    ...
    else
    {
        ...
        sprintf(buf, "&wYou have awarded %s &Y%ld&w Quest Points!\n\r", victim->name, value);
        send_to_char(buf, ch);
...

{
    ...
    {
        send_to_char("&RAre you dense or just stupid???\n\rMob's don't have skills jackass!!!&w\n\r", ch);
        return;
    }
...
void do_authorize( CHAR_DATA * ch, const char *argument )
{
   ...
   if( arg2[0] == '\0' || !str_cmp( arg2, "accept" ) || !str_cmp( arg2, "yes" ) )
   {
      ...
      ch_printf( victim,   /* B */
                 ...
                 "You are now fully authorized to play Star Wars Reality.\r\n", victim->name ); /* B */
      return;
   }
...
void do_pcrename( CHAR_DATA * ch, const char *argument )
{
   ...
   snprintf( newname, MAX_INPUT_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( arg2[0] ), capitalize( arg2 ) );
   snprintf( oldname, MAX_INPUT_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( victim->name[0] ), capitalize( victim->name ) );
   snprintf( backname, MAX_INPUT_LENGTH, "%s%c/%s", BACKUP_DIR, tolower( victim->name[0] ), capitalize( victim->name ) );
   ...
   if( IS_IMMORTAL( victim ) )
   {
      ...
      snprintf( godname, MAX_INPUT_LENGTH, "%s%s", GOD_DIR, capitalize( victim->name ) );
      remove( godname );
   }
   ...
   if( victim->pcdata->area )
   {
      ...
      char newfilename[MAX_INPUT_LENGTH];

      snprintf( filename, MAX_INPUT_LENGTH, "%s%s.are", BUILD_DIR, victim->name );
      snprintf( newfilename, MAX_INPUT_LENGTH, "%s%s.are", BUILD_DIR, capitalize( arg2 ) );
      rename( filename, newfilename );

      snprintf( filename, MAX_INPUT_LENGTH, "%s%s.are.bak", BUILD_DIR, victim->name );
      snprintf( newfilename, MAX_INPUT_LENGTH, "%s%s.are.bak", BUILD_DIR, capitalize( arg2 ) );
      rename( filename, newfilename );
   }
...
void add_social( SOCIALTYPE * social )
{
    ...
    * somewhat ugly kludge to cast away constness.
    */
