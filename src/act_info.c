...
void do_slist( CHAR_DATA * ch, const char *argument )
{
    ...
    argument = one_argument(argument, arg2);
    argument = one_argument(argument, arg3);
    chosenability = MAX_ABILITY + 1;

    if (arg1[0] != '\0')
    {
        switch (arg1[0])
        {
        case 'b': if (!str_prefix(arg1, "bounty hunting")) chosenability = HUNTING_ABILITY; break;
        case 'c': if (!str_prefix(arg1, "combat")) chosenability = COMBAT_ABILITY; break;
        case 'd': if (!str_prefix(arg1, "diplomacy")) chosenability = DIPLOMACY_ABILITY; break;
        case 'e': if (!str_prefix(arg1, "engineering")) chosenability = ENGINEERING_ABILITY; break;
        case 'f': if (!str_prefix(arg1, "force")) chosenability = FORCE_ABILITY; break;
        case 'g': if (!str_prefix(arg1, "general")) chosenability = -1; break;
        case 'l': if (!str_prefix(arg1, "leadership")) chosenability = LEADERSHIP_ABILITY; break;
        case 'm': if (!str_prefix(arg1, "medical")) chosenability = MEDICAL_ABILITY; break;
        case 'p': if (!str_prefix(arg1, "piloting")) chosenability = PILOTING_ABILITY; break;
        case 's': if (!str_prefix(arg1, "smuggling")) chosenability = SMUGGLING_ABILITY; break;
        default: chosenability = MAX_ABILITY + 1; break;
        }
    }
    lowlev = 1;
    hilev = 150;
    if (chosenability > MAX_ABILITY)
    {/* if First Argument Was not an ability. */

        if (arg1[0] != '\0')
            lowlev = atoi(arg1);

        if ((lowlev < 1) || (lowlev > 151))
            lowlev = 1;

        if (arg2[0] != '\0')
            hilev = atoi(arg2);

        if ((hilev < 0) || (hilev >= 151))
            hilev = 150;

        if (lowlev > hilev)
            lowlev = hilev;
    }
    else /* If it WAS an ability */
    {
        if (arg2[0] != '\0')
            lowlev = atoi(arg2);
    ...
    for (ability = -1; ability < MAX_ABILITY; ability++)
    {
        ...
        if (ability >= FORCE_ABILITY && !IS_IMMORTAL(ch) && !(ch->skill_level[FORCE_ABILITY] > 1))
            continue;
        if (ability >= 0)
        ...
        for (i = lowlev; i <= hilev; i++)
        {
            ...
            for (sn = 0; sn < top_sn; sn++)
            {
                ...
                {
                    pager_printf(ch, "&Y(&R%3d&Y)&C %-18.18s  ",
...
/*
short get_primary_ability( class ) //short class)
{
    switch (class)
    {
    default:
    case CLASS_COMBAT:      return COMBAT_ABILITY;
    case CLASS_PILOT:       return PILOTING_ABILITY;
    case CLASS_ENGINEERING: return ENGINEERING_ABILITY;
    case CLASS_HUNTING:     return HUNTING_ABILITY;
    case CLASS_SMUGGLING:   return SMUGGLING_ABILITY;
    case CLASS_LEADERSHIP:  return LEADERSHIP_ABILITY;
    case CLASS_DIPLOMACY:   return DIPLOMACY_ABILITY;
    case CLASS_MEDICAL:     return MEDICAL_ABILITY;
    //case CLASS_ESPIONAGE:   return ESPIONAGE_ABILITY;
    case CLASS_FORCE:       return FORCE_ABILITY;
    case CLASS_CHEMIST:     return ENGINEERING_ABILITY;
    case CLASS_SCOUNDREL:   return COMBAT_ABILITY;
    case CLASS_AMBASSADOR:  return DIPLOMACY_ABILITY;
    case CLASS_SCOUT:       return COMBAT_ABILITY;
    case CLASS_SNITCH:      return SMUGGLING_ABILITY;
    case CLASS_COMMANDO:    return COMBAT_ABILITY;
    }
}
*/
...
void do_subclass( CHAR_DATA * ch, const char *argument)
{
    ...
    if (!str_cmp(argument, "survivor"))
    {
        ...
        {
            send_to_char("You need at least level 100, before becoming a survival expert.\n\r", ch);
            return;
        }
    }
    if (!str_cmp(argument, "soldier"))
    {
        ...
        {
            send_to_char("You need at least level 100, before becoming an army of one.\n\r", ch);
            return;
        }
    }
    if (!str_cmp(argument, "mechanic"))
    {
        ...
        {
            send_to_char("You need at least level 100, before becoming an expert mechanic.\n\r", ch);
            return;
        }
    }
    ...
    if (!str_cmp(argument, "force sensitive"))
    {
        if (ch->top_level >= 99)
        {
            /*if (IS_SET(race_table[ch->race]->flags, RACE_NOFORCE))
            {
                ch_printf(ch, "Your race is restricted from receiving the force.\n\r");
                return;
            }
            if (IS_DROID(ch))
            {
                ch_printf(ch, "Droids need not apply.\n\r");
                return;
            } */
            if (ch->skill_level[/*get_primary_ability*/(ch->main_ability)] < 100)
            {
                ch_printf(ch, "But alas, you have not made your force roll yet, try again later.\n\r");
                return;
            }
            /* Force sensitive is for people who are not yet forcers,
               but we don't care if they already rolled force.
               -Lajos
             */
            if (ch->skill_level[FORCE_ABILITY] >= 2)
            {
                ch_printf(ch, "But you're already force sensitive.\n\r");
                return;
            }
            ch->perm_frc += 5;
            if (number_percent() <= 75)
                ch->perm_frc += number_range(1, 5);
            if (number_percent() <= 50)
                ch->perm_frc += number_range(1, 5);
            if (number_percent() <= 25)
                ch->perm_frc += number_range(1, 5);
            ch->perm_frc = URANGE(5, ch->perm_frc, 20);
            ch->max_mana += 700;
            ch->subclass = SUBCLASS_FORCE_SENSITIVE;
            was_in_room = ch->in_room; char_from_room(ch);
            char_to_room(ch, get_room_index(2137));
            do_clone(ch, "");
            char_from_room(ch);
            char_to_room(ch, was_in_room);
            ch_printf(ch, "You feel your soul take on a new weight.\n\r"); return;
        }
        else
        {
            ch_printf(ch, "Your soul is not yet of significant caliber to force a force roll.\n\r");
            return;
        }
    }
    if (!str_cmp(argument, "jedi"))
    {
        if (ch->top_level >= 99)
        {
            ...
            if (ch->alignment < 400)
            {
                ch_printf(ch, "You are not in tune with the nature of things enough to become a jedi.\n\r");
                return;
            }
            ch->perm_frc += 6;
            if (number_percent() <= 75)
            ...
            if (number_percent() <= 25)
                ch->perm_frc += number_range(1, 5);
            ch->perm_frc = URANGE(6, ch->perm_frc, 20);
            ch->max_mana += 1000;
            ...
            char_from_room(ch);
            char_to_room(ch, was_in_room);
            ch_printf(ch, "You feel the power of the force flow from within you.\n\r");
            return;
        }
        else
        {
    ...
    if (!str_cmp(argument, "prodigy"))
    {
        if (ch->top_level >= 99)
        {
            ...
            {
                send_to_char("You need at least level 75 levels in force abilities.\n\r", ch);
                return;
            }
            ch->subclass = SUBCLASS_PRODIGY;
            ch_printf(ch, "The force flows freely from you.\n\r");
            return;
        }
        else
        {
            ch_printf(ch, "You are not powerful enough to select that subclass.\n\r");
            return;
        }
    }
    if (!str_cmp(argument, "jedi hunter"))
    {
        if (ch->top_level >= 99)
        {
            if (ch->skill_level[FORCE_ABILITY] < 75)
            ...
            ch_printf(ch, "It is time to kick some ass, and chew some bubble gum; and I'm all out of bubble gum.\n\r");
            return;
        }
        else
        {
    ...
    if (!str_cmp(argument, "false prophet"))
    {
        if (ch->top_level >= 99)
        {
            if (ch->skill_level[FORCE_ABILITY] < 75)
            {
                send_to_char("You need at least 75 levels in force ablities.\n\r", ch);
                return;
            }
            if (ch->alignment > -400)
            {
                send_to_char("You're too pure to contemplate the darker powers. Go lay waste to someone first.\n\r", ch);
                return;
            }
            ch->subclass = SUBCLASS_FALSE_PROPHET;
            ch_printf(ch, "The force guides you, or does it?\n\r");
            return;
        }
        else
        {
    ...
    /*if (!str_cmp(argument, "jury rigger"))
    {
        if (ch->top_level >= 99)
        {
            if (ch->skill_level[ESPIONAGE_ABILITY] < 100)
            {
                send_to_char("You're not quite sure how to jury rig effectively yet, go get some practice.\n\r", ch);
                return;
            }
            ch->subclass = SUBCLASS_JURYRIGGER;
            ch_printf(ch, "Your mastery of impromptu work reveals itself.\n\r", ch);
            return;
        }
        else
        {
    ...
    if (!str_cmp(argument, "certified genius"))
    {
        if (ch->top_level >= 99)
        {
            if (ch->skill_level[MEDICAL_ABILITY] < 25)
            ...
            ch_printf(ch, "You proclaim yourself a genius, the certification is coming in the mail.\n\r");
            return;
        }
        else
        {
    ...
    if (!str_cmp(argument, "mercenary"))
    {
        if (ch->top_level >= 99)
        {
            ...
            ch_printf(ch, "Welcome to the club, now get to work.\n\r");
            return;
        }
        else
        {
    ...
    ch_printf(ch, "   %s, %s, %s, %s\n\r", subclasses[SUBCLASS_JACKOFTRADES], subclasses[SUBCLASS_MECHANIC],
                                           subclasses[SUBCLASS_SOLDIER], subclasses[SUBCLASS_SURVIVOR]);
    ch_printf(ch, "   %s, %s, %s\n\r",     subclasses[SUBCLASS_FORCE_SENSITIVE], subclasses[SUBCLASS_JEDI],
                                           subclasses[SUBCLASS_GENIUS]);
    ...
    ch_printf(ch, "   %s\n\r",             subclasses[SUBCLASS_MERCENARY]);
    ...
    ch_printf(ch, "Espionage Subclasses are as follows.\n\r");
    ch_printf(ch, "   %s\n\r",             subclasses[SUBCLASS_JURYRIGGER]);
    ch_printf(ch, "Medical Subclasses are as follows.\n\r");
    ch_printf(ch, "   %s, %s\n\r",         subclasses[SUBCLASS_DOCTOR], subclasses[SUBCLASS_PARAMEDIC]);
    ch_printf(ch, "Force subclasses are as follows.\n\r");
    ch_printf(ch, "   %s, %s, %s\n\r",     subclasses[SUBCLASS_PRODIGY], subclasses[SUBCLASS_SITH_HUNTER], subclasses[SUBCLASS_FALSE_PROPHET]);
    return;
}
...
void do_watch(CHAR_DATA* ch, char* argument)
{
    ...
    if (!str_cmp(arg1, "stop"))
    {
        if (ch->watching)
            ...
            ch_printf(ch, "You aren't watching anyplace.\n\r");
        return;
    }
        ...
        if (pexit->keyword)
        {
                ...
                && !IS_SET(pexit->exit_info, EX_WINDOW))
            {
                if (IS_SET(pexit->exit_info, EX_SECRET)
                    && door != -1)
                    send_to_char("Can't watch anything there.\n\r", ch);
                else
                    act(AT_PLAIN, "The $d is closed.", ch, NULL, pexit->keyword, TO_CHAR);
                return;
            }
    ...
    if (ch->watching)
        stop_watching(ch);
    start_watching(ch, pexit->to_room);
    return;
}

void do_version(CHAR_DATA * ch, const char *argument)
{
    send_to_char("SWR:GR Version 1.01\n\r", ch);
    send_to_char("Base SWRFUSS Version 1.4, Modified by Remi Traver\n\r", ch);
    send_to_char(" \n\r", ch);
    send_to_char("Big thank you to the SWRFUSS team for maintaining the SWR codebase\n\r", ch);
    send_to_char("SWRFUSS Version 1.4, recreated by Sampson & Kayle\n\r", ch);
    send_to_char(" \n\r", ch);
    send_to_char("Credit to SW:GD and SWL for skills integrated into SWR:GR\n\r", ch);
    send_to_char(" \n\r", ch);
    send_to_char("SW:GD Re-Release Version 1.01\n\r", ch);
    send_to_char("Base SW:GD modified by Lajos.", ch);
    send_to_char(" \n\r", ch);
    send_to_char("SWL Version 1.6.1\n\r", ch);
    send_to_char("Base SWR Modified by Arcturus, Raijen, Dejano and Shriak\n\r", ch);
    return;
}
