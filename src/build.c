...
const char *const weapon_table[WEAPON_MAX] = {
   "none",
   "vibro-axe", "vibro-blade", "lightsaber", "whip", "claw",
   "blaster", "slugthrower", "bludgeon", "bowcaster", "sword",
   "force pike", "vibro-sword", "flamethrower"
};
...
void do_oset( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !str_cmp( arg2, "affect" ) )
   {
      ...
      if( arg2[0] == '\0' || !argument || argument[0] == 0 )
      {
         ...
         send_to_char( "punch       climb       grip        scribe        brew         duck\r\n", ch );
         return;
      }
      loc = get_atype( arg2 );
      if( loc < 1 )
      ...
      case ITEM_WEAPON:
	   ...
	   size_t x;
            ...
            if( value < 0 )
            {
               ...
               send_to_char( "\r\nChoices:\r\n", ch );
               send_to_char( "   none, claw, vibro-axe, vibro-blade, lightsaber, whip, slugthrower, blaster, flamethrower, bludgeon, bowcaster, sword, force pike, vibro-sword, axe\r\n", ch );
               return;
            }
            tmp = 3;
            break;
...
void do_installarea( CHAR_DATA * ch, const char *argument )
{
   ...
   for( tarea = first_build; tarea; tarea = tarea->next )
   {
      if( !str_cmp( tarea->filename, arg ) )
      {
         ...
         snprintf( buf, 256, "%s%s.installed", BUILD_DIR, tarea->filename );
         snprintf( arg, 256, "%s%s", BUILD_DIR, tarea->filename );
         rename( arg, buf );
         send_to_char( "Done.\r\n", ch );
         return;
      }
