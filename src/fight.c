...
extern void gain_honour(CHAR_DATA* ch, int hincrease);
extern void loose_honour(CHAR_DATA* ch, int hdecrease);
...
void violence_update( void )
{
   ...
   for( ch = last_char; ch; lst_ch = ch, ch = gch_prev )
   {
      ...
      {
         bug( "%s: ERROR: first_char->prev != NULL, fixing...", __func__ );
         ch->prev = NULL;
      }
      ...
      if (!IS_NPC(ch) && ch->subclass == SUBCLASS_JEDI) /* Jedi are not allowed evil alignments. */
          ch->alignment = URANGE(400, ch->alignment, 1000);
      if (!IS_NPC(ch) && (ch->subclass == SUBCLASS_SITH_HUNTER || ch->subclass == SUBCLASS_FALSE_PROPHET)) /* Sith are not allowed good alignments. */
          ch->alignment = URANGE(-1000, ch->alignment, -400);
...
ch_ret one_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   /* Arcturus' version of an MA simply hits harder, :P, remind you of a weapon's damage? */
   if (ch->subclass == SUBCLASS_MARTIST && !wield)
       dam = number_range(280, 380);
   /* Calculate Racial Unarmed Bonuses */
   if (ch->race == RACE_DEFEL && !wield)
       dam += number_range(50, 75);
   else if (ch->race == RACE_WOOKIEE && !wield)
       dam += number_range(45, 55);
   else if (ch->race == RACE_NOGHRI && !wield)
       dam += number_range(35, 45);
   else if (ch->race == RACE_DUINUOGWUIN && !wield)
   {
       if (GET_AGE(ch) < 100)
           dam += number_range(45, 55);
       else if (GET_AGE(ch) >= 100 && GET_AGE(ch) <= 150)
           dam += number_range(55, 70);
       else if (GET_AGE(ch) >= 150)
           dam += number_range(70, 95);
   }
   /*
    * Bonuses.
    */
   ...
   if( !IS_AWAKE( victim ) )
      dam *= 2;
   if (dt == gsn_backstab)
       dam *= ((ch->skill_level[HUNTING_ABILITY] / 10)); //- ( victim->skill_level[COMBAT_ABILITY] / 4 ), 30 ) / 8 );

   if (dt == gsn_circle)
       dam *= ((ch->skill_level[HUNTING_ABILITY] / 20)); //- ( victim->skill_level[COMBAT_ABILITY] / 4 ), 30 ) / 16 );
   if ((dt == gsn_backstab || dt == gsn_circle) && ch->subclass == SUBCLASS_STEALTH_HUNT)
       dam *= 1.5;
   plusris = 0;
   ...
   if (dt == (TYPE_HIT + WEAPON_SLUGTHROWER) && wield && wield->item_type == ITEM_WEAPON)
   {
       if (wield->value[4] < 1)
       {
           act(AT_YELLOW, "$n points their gun at you but nothing happens.", ch, NULL, victim, TO_VICT);
           act(AT_YELLOW, "*CLICK* ... your gun needs a new clip!", ch, NULL, victim, TO_CHAR);
           if (IS_NPC(ch))
           {
   ...
   else if (dt == (TYPE_HIT + WEAPON_VIBRO_SWORD) && wield && wield->item_type == ITEM_WEAPON)
    {
    if (wield->value[4] < 1)
    {
        act(AT_YELLOW, "Your Vibro-Sword needs recharging ...", ch, NULL, victim, TO_CHAR);
        dam /= 1.5;
    }
    else
        dam *= 1.25;
    }
   else if (dt == (TYPE_HIT + WEAPON_FLAMETHROWER) && wield && wield->item_type == ITEM_WEAPON)
    {
    if (wield->value[4] < 1)
    {
        act(AT_YELLOW, "$n points his flamethrower at you, but the flame does not move.", ch, NULL, victim, TO_VICT);
        act(AT_YELLOW, "You need to refuel your flamethrower, it does not have a flame.", ch, NULL, victim, TO_CHAR);
        if (IS_NPC(ch))
        {
            do_remove(ch, wield->name);
        }
...
ch_ret damage( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( dam && dt != TYPE_UNDEFINED )
   {
      ...
      else if( dt == ( TYPE_HIT + 1 ) || dt == ( TYPE_HIT + 3 ) || dt == ( TYPE_HIT + 4 ) || dt == (TYPE_HIT + 10)
          || dt == (TYPE_HIT + 12) || dt == (TYPE_HIT + 13))
         dam = ris_damage( victim, dam, RIS_SLASH );
      else if( dt == ( TYPE_HIT + 7 ))
         dam = ris_damage( victim, dam, RIS_FIRE);
      else if ( dt == ( TYPE_HIT + 6) || dt == ( TYPE_HIT + 9 ))
         dam = ris_damage(victim, dam, RIS_ENERGY);
   ...
   if( victim != ch )
   {
      ...
      if( dt >= TYPE_HIT )
      {
         ...
         {
             int dchance = 0;
             dchance = IS_NPC(victim) ? 0
                 : (int)victim->pcdata->learned[gsn_deflect];
             if (dchance && !IS_NPC(victim))
             {
                 OBJ_DATA* victwield;
                 if (!(dt == (TYPE_HIT + WEAPON_BLASTER)))
                     return rNONE;
                 victwield = get_eq_char(victim, WEAR_WIELD);
                 if (victwield->value[3] != WEAPON_LIGHTSABER)
                     return rNONE;
                 dchance /= 2;
                 if (number_percent() < dchance)
                 {
                     damage(victim, ch, dam, gsn_deflect);
                     learn_from_success(victim, gsn_deflect);
                 }
                 else
                     learn_from_failure(victim, gsn_deflect);
             }
             return rNONE;
         }
   ...
   // send immortals to their recall and echo a congrats to their killer 
   // ...but not by mobs
   //
   // - Raijen
       ...
       && victim->hit < 1 && !IS_NPC(ch))
   {
       ...
       sprintf(buf, "%s has defeated the immortal %s!", ch->name, victim->name);
   ...
   if (!IS_NPC(victim)
       ...
       && victim->spar != 0
       && ch->spar != 0
       && victim->hit < 1 && !IS_NPC(ch) && !(victim == ch))
   {
       victim->hit = 1;
       stop_fighting(victim, TRUE);
       stop_fighting(ch, TRUE);
       send_to_char("&RYou are Defeated!!!", victim);
       send_to_char("&GYou are Victorious!!!", ch);
       if (ch->pcdata->status - victim->pcdata->status < 5)
       {
           gain_honour(ch, 1);
           loose_honour(victim, 1);
...
{
   CHAR_DATA *victmp;
   OBJ_DATA *corpse_to_return = NULL; //fix noted on smaugmuds.afkmods
   ...
   snprintf( buf, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( arg[0] ), capitalize( arg ) );
   snprintf( buf2, MAX_STRING_LENGTH, "%s%c/%s", BACKUP_DIR, tolower( arg[0] ), capitalize( arg ) );

   rename( buf, buf2 );

   snprintf( buf, MAX_STRING_LENGTH, "%s%c/%s.clone", PLAYER_DIR, tolower( arg[0] ), capitalize( arg ) );
   snprintf( buf2, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR, tolower( arg[0] ), capitalize( arg ) );
...
void group_gain( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( members == 0 )
   {
      bug( "%s: members = 0", __func__, members );
      members = 1;
   }
   ...
   for( gch = ch->in_room->first_person; gch; gch = gch_next )
   {
      ...
      {
         ch_printf( gch, "You receive %d combat experience.\r\n", xp );
      }
      ...
      if( lch == gch && members > 1 )
      {
         ...
         ch_printf( gch, "You get %d leadership experience for leading your group to victory.\r\n", xp );
...
make it simple instead */
  ...
  /*SWRFUSS original
    ...
    */
    ...
    int align, adjustalign, mod;
    int min = -1000, max = 1000;

    adjustalign = (int)(victim->alignment / 100);

    align = abs(gch->alignment + victim->alignment);
    mod = align / 400;
    ...
    if (!IS_NPC(gch) && gch->subclass == SUBCLASS_JEDI)
        min = 400;
    if (!IS_NPC(gch) && gch->subclass == SUBCLASS_SITH_HUNTER)
        max = -400;
    return URANGE(min,
        (int)(gch->alignment - adjustalign),
        max);
}
...
void dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( ch->skill_level[COMBAT_ABILITY] >= 100 )
   {
      ...
      snprintf( tmp, MAX_INPUT_LENGTH, "%s You do %d points of damage.", buf2, dam );
      mudstrlcpy( buf2, tmp, 256 );
   }
...
void do_kill( CHAR_DATA * ch, const char *argument )
{
   ...
   if (!IS_NPC(ch))
   {
       ...
       {
           send_to_char("You are a little bound up at the moment.\n\r", ch);
           return;
       }
   }
...
void do_flee( CHAR_DATA * ch, const char *argument )
{
   ...
   for( attempt = 0; attempt < 8; attempt++ )
   {
      ...
      send_to_char( "You run for cover!\r\n", ch );

      stop_fighting( ch, TRUE );
      return;
   }

   send_to_char( "You attempt to run for cover!\r\n", ch );
}
