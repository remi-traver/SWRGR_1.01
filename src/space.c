    ...
    for (ship = first_ship; ship; ship = ship->next)
    {
        ...
        if (!str_cmp(ship->owner, ""))
            ch_printf(ch, "%-53s %-15s %ld\n\r", ship->name,
                ship->ship_class == FIGHTER_SHIP ? "(Starfighter)" :
                (ship->ship_class == MIDSIZE_SHIP ? "(Midtarget)" :
                    (ship->ship_class == CAPITAL_SHIP ? "(Capital Ship)" :
                        (ship->ship_class == SHIP_PLATFORM ? "(Platform)" :
                            (ship->ship_class == CLOUD_CAR ? "(Cloudcar)" :
                                (ship->ship_class == FRIGATE_SHIP ? "(Frigate)" :
                                    (ship->ship_class == LAND_SPEEDER ? "(Speeder)" :
                                        (ship->ship_class == SUPERCAPITAL_SHIP ? "(SuperCapital)" :
                                            (ship->ship_class == MOBILE_SUIT ? "(Coralskipper)" :
                                                (ship->ship_class == WALKER ? "(Walker)" : "(Unknown)"))))))))),
                get_ship_value(ship));
        count++;
    }
...
void do_availships( CHAR_DATA * ch, const char* argument)
{
    SHIP_DATA* ship;
    int count = 0;
...
void do_detach( CHAR_DATA * ch, const char *argument)
{
    ...
    if ((ship->ship_class == FRIGATE_SHIP || ship->ship_class == CAPITAL_SHIP || ship->ship_class == MOBILE_SUIT) && docked_by->ship_class > ship->ship_class)
    {
        price += (ship->maxhull - ship->hull);
        price += (50 * (ship->maxmissiles - ship->missiles));
        price += (75 * (ship->maxtorpedos - ship->torpedos));
        ...
        if (ch->pcdata && ch->pcdata->clan && !str_cmp(ch->pcdata->clan->name, ship->owner))
        {
            ...
            ch->pcdata->clan->funds -= price;
            ch_printf(ch, "&GIt costs %s %d credits to repair this ship for launch.\n\r", ch->pcdata->clan->name, price);
        }
        else if (str_cmp(ship->owner, "Public"))
        {
            if (ch->gold < price)
            ...
            ch->gold -= price;
            ch_printf(ch, "&GYou pay %d credits to ready the ship for launch.\n\r", price);
        }
...
/*Sabotage by Arcturus */  //Commented out
    ...
    default:
        ...
        return;
                case 1:
                    if ( !ch->dest_buf )
                        return;
                    mudstrlcpy( arg, ch->dest_buf ); //changed strcpy to mudstrlcpy
                    DISPOSE(ch->dest_buf);
                    break;
                case SUB_TIMER_DO_ABORT:
                    DISPOSE(ch->dest_buf);
