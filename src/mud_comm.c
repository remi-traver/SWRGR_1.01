...
void do_mpsleep(CHAR_DATA * ch, const char *argument)
{
    char arg[MAX_STRING_LENGTH];
    //CHAR_DATA *victim;
    int beats;
    ...
    if (arg[0] == '\0' || !is_number(arg))
    {
        progbug("Mpsleep - Non-numeric used as argument", ch);
        return;
    }

    beats = atoi(arg);
    beats = UMAX(beats, 1);
    beats *= PULSE_PER_SECOND;
...
/*
 * syntax: mpcast "spell" "level" "target"
 */
void do_mp_cast(CHAR_DATA * ch, const char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    CHAR_DATA* victim, * actor;
    int level, sn;
    OBJ_DATA* obj = NULL;
    ...
    if (arg1[0] == '\0')
    {
        send_to_char("mpcast what?\n\r", ch);
        progbug("Mpcast: invalid argument1", ch);
        return;
    }

    if (arg2[0] == '\0')
    {
        send_to_char("mpcast, what level?\n\r", ch);
        progbug("Mpcast: invalid argument2", ch);
        return;
    }
    if (argument[0] == '\0')
    {
        send_to_char("mpcast at whom?\n\r", ch);
        progbug("Mpcast: invalid target.", ch);
        return;
    }
    actor = ch;
    if (IS_NPC(ch) && ch->pIndexData->vnum == 3)
    {
        ...
        {
            obj = ch->supermob;
            if (ch->supermob->carried_by)
                actor = ch->supermob->carried_by;
        }
    }
    ...
    {
        send_to_char("Victim must be in room.\n\r", ch);
        progbug("Mpcast: victim not in room", ch);
        return;
    }
    if (!is_number(arg2))
    {
        progbug("mpcast: invalid level, using caster level.", ch);
        level = ch->top_level;
    }
    else
    {
        ...
        if (level < 0)
        {
            progbug("mpcast: negative level, using caster level.", ch);
            level = ch->top_level;
        }
    }
    ...
    if (is_number(arg1))
    {
        ...
        if (sn > gsn_top_sn)
        {
            progbug("mpcast: invalid skill.", ch);
            return;
        }
    }
    else
    ...
    {
        progbug("mpcast: Skill specified is not a spell.", ch);
        return;
    }

    obj_cast_spell(sn, level, actor, victim, obj);
    return;
}

void do_mp_alignshift(CHAR_DATA * ch, const char *argument)
{
    int alignshift, min, max;
    min = -1000, 
    max = 1000;
    char arg1[MAX_INPUT_LENGTH];
    CHAR_DATA* victim;
    bool valid_shift = FALSE;
    ...
    if (arg1[0] == '\0')
    {
        send_to_char("mpalignshift whom?\n\r", ch);
        progbug("Mpalignshift: invalid target", ch);
        return;
    }

    if (argument[0] == '\0')
    {
        send_to_char("mpalignshift, to what degree?\n\r", ch);
        progbug("Mpalignshift: invalid degree", ch);
        return;
    }
    ...
    {
        send_to_char("Victim must be in room.\n\r", ch);
        progbug("mpalignshift: victim not in room", ch);
        return;
    }
    ...
    if (!valid_shift)
    {
        send_to_char("mpalignshift: invalid degree.\n\r", ch);
        progbug("mpalignshift: invalid degree", ch);
        return;
    }
    else
        alignshift = atoi(argument);

    if (!IS_NPC(victim) && victim->subclass == SUBCLASS_JEDI)
        min = 400;
    if (!IS_NPC(victim) && victim->subclass == SUBCLASS_SITH_HUNTER)
        max = 400;
    victim->alignment = URANGE(min, victim->alignment + alignshift, max);
    return;
}

void do_mpbounty(CHAR_DATA * ch, const char *argument)
{
    CHAR_DATA* victim;
    ...
    {
        send_to_char("Victim must be in room.\n\r", ch);
        progbug("mpbounty: victim not in room", ch);
        return;
    }
    ...
    if (!IS_SET(victim->pcdata->flags, PCFLAG_BOUNTY_SPEC))
        SET_BIT(victim->pcdata->flags, PCFLAG_BOUNTY_SPEC);
    return;
}
...
void  do_mpaward(CHAR_DATA * ch, const char *argument)
{
    ...
    if (arg1[0] == '\0')
    {
        ...
        progbug("Mpaward: null target", ch);
        return;
    }

    if (arg2[0] == '\0')
    {
        send_to_char("mpaward how much?\n\r", ch);
        progbug("Mpaward: null qp value", ch);
        return;
    }
    ...
    ch_printf(victim, "You gain %d QPS!!\n\r", qpgain);
    victim->pcdata->quest_accum += qpgain;
    victim->pcdata->quest_curr += qpgain;
    return;
}
...
void  do_mpgain(CHAR_DATA * ch, const char *argument)
{
    ...
    xp_cap = UMAX(500, exp_level(victim->skill_level[ability] + 1) - exp_level(victim->skill_level[ability]));

    exp = URANGE(1, exp, xp_cap);
