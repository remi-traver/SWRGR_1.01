...
void do_newscore( CHAR_DATA * ch, const char *argument)
{
    ...
    ch_printf(ch, "&BDamroll&z:   &C%-24d &BBank&O:      &C%ld\n\r", GET_DAMROLL(ch), ch->pcdata->bank);
    ...
    ch_printf(ch, "&z&w/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\\n\r");
    {
        ...
        for (ability = 0; ability < MAX_ABILITY; ability++)
            if (ability < FORCE_ABILITY || ch->skill_level[FORCE_ABILITY] > 1)
                ch_printf(ch, "&B%-15s  &CLevel&z:&c %-3d  &C Max&z:&c %-3d   &CExp&z:&c %-10ld  &C Next&z:&c %-10d\n\r",
                ...
                ch_printf(ch, "&B%-15s  &CLevel&z:&c %-3d  &C Max&z:&c %-3d   &CExp&z:&c %-10ld  &C Next&z:&c %-10d\n\r",
                    ability_name[ability], ch->skill_level[ability], max_level(ch, ability), ch->experience[ability],
                    exp_level(ch->skill_level[ability] + 1));
                /*ch_printf(ch, "&B%-15s  &CLevel&z:&c %-3d  &C Max&z:&c ???   &CExp&z:&c ???      &C Next&z:&c ???\n\r",
...
/* Borrowed styles from SWL code base  */
void do_style( CHAR_DATA * ch, const char *argument )
{
    ...
    argument = one_argument(argument, arg);
    //one_argument(argument, arg2); //added, similar to void do_pcrename
    smash_tilde(arg);   //added, similar to void do_pcrename
...
void do_qpbuy( CHAR_DATA * ch, const char *argument )
{
    ...
    argument = one_argument( argument, arg );
    one_argument( argument, arg2 ); //added, similar to void do_pcrename
    smash_tilde( arg2 );   //added, similar to void do_pcrename
    ...
    if (!str_cmp(arg, "force"))
    {
        ...
        if (ch->perm_frc >= 20)  //used to set limit for QPbuy frc
        {
            send_to_char("You can't improve your force over 100 levels with questpoints.\n\r", ch);
            return;
        }
    ...
    if (!str_cmp(arg, "rship"))    // spec - rename ship with qp
    {
        ...
        smash_tilde( arg ); // Removes ~s, just in case ruins save ship.
