...
{
    send_to_char("&RThis command is currently disabled!!!\n\r&w", ch);
    return;
}
...
/* From SWGD  */
void do_force_roll( CHAR_DATA *ch) 
//, const char *argument )
{
    //int pcnt1 = 0, pcnt2 = 0, pcnt3 = 0, pcnt4 = 0;
    if (IS_NPC(ch))
        return;

    /* Arcturus Random Force Roll */
    if (ch->main_ability != CLASS_HUNTING && ch->main_ability != CLASS_SNITCH
        && ch->perm_frc <= 0)
    {
        /* Old Force Roll Structure
        if (number_percent() <= 15)
        {
           ...
           if(number_percent() <= 75 )
               ch->perm_frc   += number_range(1, 5);
           ch->perm_frc = URANGE( 1, ch->perm_frc, 20 ); // 1-20 is acceptable frc stat range
        }
        */
        /* Updated force roll installed 3/10/2014 -Lajos.  Further updated by Remi 5/16/2020 */
        if (number_percent() <= 10)
        {
            if (number_percent() <= 40)
            {
                ch->perm_frc = number_range(1, 10) + number_range(1, 10) + number_range(1, 5);
                ch->perm_frc = URANGE(5, ch->perm_frc, 20); // add 5 to first slot = produce nobody less than level 25, even if they roll below it.
            }
            else
            {
                ch->perm_frc = number_range(1, 5) + number_range(1, 5) + number_range(1, 5);
                ch->perm_frc = URANGE(5, ch->perm_frc, 20); // add 5 to first slot = produce nobody less than level 25, even if they roll below it.
            }
        }
    }
    ...
    {
        ch->max_mana = 600;
        sprintf(log_buf, "%s is a forcer (%d).", ch->name, ch->perm_frc);
        log_string_plus(log_buf, LOG_COMM, UMAX(sysdata.log_level, ch->top_level));
        snprintf(log_buf, MAX_STRING_LENGTH, "%s is a forcer (%d).", ch->name, ch->perm_frc);
        to_channel(log_buf, CHANNEL_MONITOR, "Monitor", 1);
    }
}

/* Remi Traver - Wiz ability for forcing force roll*/
void do_force_rollaward( CHAR_DATA *ch, const char *argument)
{
    CHAR_DATA *victim;
    ...
    if (arg1[0] == '\0' )
    {
        send_to_char("Syntax:Force_roll who?.\n\r", ch);
        return;
    }
    ...
    if (IS_NPC(victim))
    {
        send_to_char("&YNot on NPC's!\n\r", ch);
        return;
    }

    if (ch == victim)
    {
        send_to_char("&YYou may not award yourself!", ch);
        return;
    }

    if (IS_IMMORTAL(victim))
    {
        send_to_char("&RYou cannot award an immortal!\n\r", ch);
        return;
    }

    if (victim->forsaken != 0)
    {
        send_to_char("&RNot on forsaken people.\n\r", ch);
        return;
    }
    
    do_force_roll( victim )
    ;
...
void do_hail( CHAR_DATA * ch, const char *argument )
{
   ...
   if (!IS_NPC(ch))
   {
       ...
       {
           send_to_char("You really think a Taxi is going to pick you up while bound?\n\r", ch);
           return;
       }
   }
...
void do_train( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         add_timer( ch, TIMER_DO_FUN, 1, do_train, 1 ); //changed from 10 to 1
...
/* New Version- Arcturus, Old version Removed.*/
void do_changes( CHAR_DATA* ch, const char *argument)
{
    ...
    FILE *fp;
    send_to_pager_color("\n\r&RChange                                                            &w|&RDate\n\r&w", ch);
    send_to_pager_color("&B------------------------------------------------------------------&w|&B--------\n\r&w", ch);
    sprintf(filename, "%schanges.txt", SYSTEM_DIR);
    if ((fp = fopen(filename, "r")) != NULL)
    {
        while (!feof(fp))
        {
            if (feof(fp))
                break;
            line = fread_line(fp);
            pager_printf(ch, "%s\r", line);
        }
        fclose(fp);
    }
    else
    {
        send_to_char("Changes file appears to be empty, or does not exist.\n\r", ch);
        return;
    }
}

/* probably could have used the same command, as before, but was too lazy, Oldchanges it is.
 * I'm going to use it to store really old changes, in case people are still interested, the normal
 * changes list is getting a little long, and people are complaining that it is spam. ~Arcturus
 */
void do_oldchanges(CHAR_DATA * ch, const char *argument)
{
    char* line;
    char filename[256];
    FILE* fp;
    send_to_pager_color("\n\r&RChange                                                            &w|&RDate\n\r&w", ch);
    send_to_pager_color("&B------------------------------------------------------------------&w|&B--------\n\r&w", ch);
    sprintf(filename, "%soldchanges.txt", SYSTEM_DIR);
    if ((fp = fopen(filename, "r")) != NULL)
    {
        while (!feof(fp))
        {
