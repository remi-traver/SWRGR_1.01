...
void do_force_roll args((CHAR_DATA * ch));
...
void nanny_stats_ok( DESCRIPTOR_DATA *d, const char *argument )
{
  ...
  switch( argument[0] )
    ...
    case 'n':
    case 'N':
      ch->perm_str = number_range( 3, 6 ) + number_range( 1, 6 ) + number_range( 1, 6 );
      ch->perm_int = number_range( 3, 6 ) + number_range( 1, 6 ) + number_range( 1, 6 );
      ch->perm_wis = number_range( 3, 6 ) + number_range( 1, 6 ) + number_range( 1, 6 );
      ch->perm_dex = number_range( 3, 6 ) + number_range( 1, 6 ) + number_range( 1, 6 );
      ch->perm_con = number_range( 3, 6 ) + number_range( 1, 6 ) + number_range( 1, 6 );
  ...
  if( ch->top_level == 0 )
    {
      ...
      ch->defensive_skill = 0;
      ch->agressive_skill = 0;
      ch->evasive_skill = 0;
      ch->offensive_skill = 0;
      
      /* SWRFUSS Lck and Frc rolls */
      ch->perm_lck = number_range( 6, 18 );
      ch->perm_frc = number_range( -2000, 20 );  //leaving SWRFUSS roll in as second chance
      ch->affected_by = race_table[ch->race].affected;
      ch->perm_lck += race_table[ch->race].lck_plus;
      ch->perm_frc += race_table[ch->race].frc_plus;

      ch->force_roll = number_range( 100, 100);
      ...
      if (ch->perm_frc > 20)
       ...
       End Arcturus Random Force Roll */

      /* Force levels for Race */
	...
	ch->perm_frc = URANGE( 0, ch->perm_frc, 20 );
      ...
      /* Forcer Check and Announce*/
      if (ch->perm_frc >= 1)
      {
          sprintf(log_buf, "New Character %s is a forcer (%d).", ch->name, ch->perm_frc);
          log_string_plus(log_buf, LOG_COMM, UMAX(sysdata.log_level, ch->top_level));
      }
	...
	ch->max_mana = 0;
      ...
      if( ch->main_ability == CLASS_AMBASSADOR )
      {
          ch->perm_frc = 10;
          ch->max_mana = 600;
      }
...
void act( short AType, const char *format, CHAR_DATA * ch, const void *arg1, const void *arg2, int type )
{
  const char *txt;
   ...
   if( type == TO_VICT )
   {
      if( !vch )
      {
         bug( "%s: null vch with TO_VICT. %s(%s)", __func__, ch->name, format );
         return;
      }
      if( !vch->in_room )
      {
...
void do_name( CHAR_DATA * ch, const char *argument )
{
   ...
   snprintf( fname, 256, "%s%c/%s", PLAYER_DIR, tolower( ucase_argument[0] ), capitalize( ucase_argument ) );
   if( stat( fname, &fst ) != -1 )
   {
      send_to_char( "That name is already taken.  Please choose another.\r\n", ch );
      return;
