...
/* Cutdoor is just like doorbash, Ezekiel */
/*
void do_cutdoor( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch )
    &&  ch->pcdata->learned[gsn_cutdoor] <= 0  )
    {
        send_to_char( "You're not enough of a Jedi to cut through doors!\n\r", ch );
        return;
    }
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Cut through what?\n\r", ch );
        return;
    }

    if ( ch->fighting )
    {
        send_to_char( "You are using all your hands for fighting!\n\r", ch );
        return;
    }
 ...
 if ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL
    ...
    {
    send_to_char( "It helps when you wield your lightsaber!!!\n\r", ch );
    return;
    }
    ...
    if ( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
    {
        ...
        if ( !IS_SET( pexit->exit_info, EX_CLOSED ) )
        {
        send_to_char( "Chill dude! The door is open! Why waste your blade?\n\r", ch );
        return;
        }
        ...
        act(AT_SKILL, "You burn a HUGE whole in the $d with your lightsaber!", ch, NULL, keyword, TO_CHAR );
        act(AT_SKILL, "$n burns a HUGE hole in the $d with their lightsaber!",          ch, NULL, keyword, TO_ROOM );
        ...
        {
        act(AT_SKILL, "You press your blade into the door but the metal withholds",
            ch, NULL, keyword, TO_CHAR );
        act(AT_SKILL, "$n slashes against the $d, but it holds strong.",
    ...
    else
    {
        ...
        act(AT_SKILL, "$n slashes against the door, but it holds strong..",
