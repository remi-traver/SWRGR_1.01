...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover )
{
   ...
   if( stat( strsave, &fst ) != -1 )
   {
      ...
      {
         snprintf( buf, MAX_INPUT_LENGTH, "%s player data for: %s (%dK)",
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'S':
            ...
            KEY( "SWeapon", ch->sniperweapon, fread_number( fp ) );
...
void do_last( CHAR_DATA * ch, const char *argument )
{
   ...
   if( stat( filename, &fst ) != -1 && check_parse_name( capitalize( arg ) ) )
      ch_printf( ch, "%s was last on: %s\r\n", arg, ctime( &fst.st_mtime ) );
   else
      ch_printf( ch, "%s was not found.\r\n", arg );
}


/* parnic's saveall...help simplify copyover */
/*
void do_saveall( CHAR_DATA * ch, const char *argument)
{
    PLANET_DATA *p;
    CLAN_DATA *c;
    CHAR_DATA *vch;
    ...
    send_to_char("&gSaving &Gplanets&g...\n\r", ch);
    for (p = first_planet; p; p = p->next)
        save_planet(p);
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("&gSaving &Gclans&g...\n\r", ch);
    for (c = first_clan; c; c = c->next)
        save_clan(c);
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("&gSaving &Gplayers&g...\n\r", ch);
    for (vch = first_char; vch; vch = vch->next)
        save_char_obj(vch);
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("&gSaving &Gships&g...\n\r", ch);
    for (sh = first_ship; sh; sh = sh->next)
        save_ship(sh);
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("&gSaving &Gstarsystems&g...\n\r", ch);
    for (s = first_starsystem; s; s = s->next)
        save_starsystem(s);
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("&gSaving &Gcommands&g...\n\r", ch);
    save_commands();
    send_to_char("       &G&cDone.\n\r", ch);
    send_to_char("&gSaving &Gskills&g...\n\r", ch);
    save_skill_table();
    send_to_char("       &G&cDone.\n\r", ch);
    send_to_char("&gSaving &Gherbs&g...\n\r", ch);
    save_herb_table();
    send_to_char("       &G&cDone.\n\r", ch);
    send_to_char("&gSaving &Gsocials&g...\n\r", ch);
    save_socials();
    send_to_char("       &G&cDone.\n\r", ch);
    send_to_char("&gSaving &Gareas&g...\n\r", ch);
    do_foldarea(NULL, "all");
    send_to_char("       &G&cDone.\n\r", ch);
    send_to_char("&gSaving &Ghelp files&g...\n\r", ch);
    do_hset(NULL, "save");
    send_to_char("       &G&cDone.\n\r", ch);

    send_to_char("\n\r&G&WSaveall completed succesfully.\n\r", ch);
