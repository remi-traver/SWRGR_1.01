...
/* skill apply types	-Thoric */
      ...
      case APPLY_DODGE:
         ...
         break;
      case APPLY_DUCK:
          if (!IS_NPC(ch) && ch->pcdata->learned[gsn_duck] > 0)
              ch->pcdata->learned[gsn_duck] = UMAX(1, ch->pcdata->learned[gsn_duck] + mod);
          break;
      case APPLY_PEEK:
      ...
      case APPLY_GRIP:
         ...
         break;
      case APPLY_RESEARCHTARGET:
          if (!IS_NPC(ch) && ch->pcdata->learned[gsn_researchtarget] > 0)
              ch->pcdata->learned[gsn_researchtarget] = UMAX(1, ch->pcdata->learned[gsn_researchtarget] + mod);
          break;
...
const char *extra_bit_name( int extra_flags )
{
   ...
   if (extra_flags & ITEM_TWO_HANDED)
       mudstrlcat(buf, " two handed", MAX_STRING_LENGTH);
   ...
   if ( extra_flags & ITEM_STOLEN ) 
       mudstrlcat( buf, " stolen", MAX_STRING_LENGTH );
 ...
 */
   ...
   if( ( number_percent(  ) - get_curr_lck( ch ) + 13 - ms ) )
      return TRUE;
   else
      return FALSE;
}
...
/* Scryn, standard luck check + consideration of 1 attrib 2/2/96*/
   ...
   if( number_percent(  ) - get_curr_lck( ch ) + 13 - attrib + 13 )
      return TRUE;
   else
      return FALSE;
}
...
void stop_watching(CHAR_DATA* ch)
{
    ...
    if (ch->watching->name)
        sprintf(buf, "You stop watching %s.\n\r", ch->watching->name);
    else
        sprintf(buf, "You stop watching that room.\n\r");

    /*ch_printf(ch, buf);
    
        act( AT_ACTION, buf, ch, NULL, NULL, TO_CHAR );
     */
    UNLINK(ch, ch->watching->first_watcher, ch->watching->last_watcher,
        next_watcher, prev_watcher);

    ch->watching = NULL;
    ch->next_watcher = NULL;
    ch->prev_watcher = NULL;

}


void start_watching(CHAR_DATA* ch, ROOM_INDEX_DATA* room)
{
    char buf[MAX_STRING_LENGTH];
    ...
    if (!room)
        return;
    if (room->name)
        sprintf(buf, "You start watching %s.", room->name);
    else
        sprintf(buf, "You start watching that room.");
    act(AT_ACTION, buf, ch, NULL, NULL, TO_CHAR);

    LINK(ch, room->first_watcher, room->last_watcher, next_watcher, prev_watcher);
    ch->watching = room;
    return;
}
